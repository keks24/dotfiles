--[[
from "mpv/player/lua/select.lua"
...
local function format_playlist_entry(entry, show)
    --local item = entry
    -- show absolute path for each item. the real-time-search will be slow!
    local item = entry.filename
    -- to only show the parent directory for each item.
    --local directory, filename = utils.split_path(entry.filename)
    --local parent_directory = directory and directory:match("([^/]+)/[/]*$")
    --local item = parent and (parent .. "/" .. filename) or filename
...
-- custom - 20260331T122758+0200 - rfischer: select playlist entry via "g-p".
mp.add_key_binding("g-p", "select-playlist", function ()
    local playlist = {}
    local default_item
